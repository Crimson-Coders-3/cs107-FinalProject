...
SRCPATH=${CURRENT_PATH}/build/core/src/CMakeFiles/Roots.dir
TESTPATH=${CURRENT_PATH}/build/core/tests/src/CMakeFiles/ROOTSTest.dir
...
if [ -d "${TESTPATH}" ]; then
    lcov --capture -d ${SRCPATH} -d ${TESTPATH} -o ROOTS_COV.info
else
...
lcov --remove ROOTS_COV.info "/usr/*" "${INSTALL_PATH}/*" "${TEST_SRC_PATH}/*" -o ROOTS_COV.info

# generate html version of coverage
genhtml ROOTS_COV.info -o CODE_COVERAGE
cd ..
