...
# library sources
# =============== 
ROOTS_DIRECTORY=${PROJECT_ROOT}/Roots
ROOTS_SO_NAME=Roots
...
# ====================== 
INSTALL_DIRECTORY=${ROOTS_DIRECTORY}/install
INSTALL_ROOTS_DIRECTORY=${INSTALL_DIRECTORY}

BUILD_DIRECTORY=${ROOTS_DIRECTORY}/build
BUILD_ROOTS_DIRECTORY=${BUILD_DIRECTORY}
...
# ====================================== 
COMPILE_INSTALL_ROOTS_DIRECTORY="${INSTALL_ROOTS_DIRECTORY}"
COMPILE_BUILD_ROOTS_DIRECTORY="${BUILD_ROOTS_DIRECTORY}"
...
# ====================== 
if [ ! -d "${ROOTS_DIRECTORY}" ]; then
  echo " "
  echo "Error:"
  echo "${ROOTS_DIRECTORY} does not exist."
  exit 1
fi
...
if [ $BUILD_CLEAN == 1 ]; then
  echo " "
  echo "Clean: removing ${COMPILE_BUILD_ROOTS_DIRECTORY}"
  echo "Clean: removing ${COMPILE_INSTALL_ROOTS_DIRECTORY}"
  echo "Clean: removing coverage"
  echo " "
  rm -rf $COMPILE_BUILD_ROOTS_DIRECTORY
  rm -rf $COMPILE_INSTALL_ROOTS_DIRECTORY
  rm -rf coverage
fi
...
if [ $BUILD_ROOTS == 1 ]; then
  echo " "
  echo -e "${mC} ========== Building Roots ========= ${eC}"
  echo "   Compiling Options:"
  echo "          Build Type: ${BUILD_TYPE}"
  echo "    Install Location: ${COMPILE_INSTALL_ROOTS_DIRECTORY}"
  echo " Executable Location: ${COMPILE_BUILD_ROOTS_DIRECTORY}/bin"
  echo " "
  echo "                  CC: ${CC}"
  echo "                 CXX: ${CXX}"
  echo " "
  ...
  if [ ! -d $COMPILE_BUILD_ROOTS_DIRECTORY ]; then
    mkdir $COMPILE_BUILD_ROOTS_DIRECTORY
  fi
  cd $COMPILE_BUILD_ROOTS_DIRECTORY

  cmake -D CMAKE_C_COMPILER=${CC_PATH}                              \
        -D CMAKE_CXX_COMPILER=${CXX_PATH}                           \
        -D CMAKE_C_FLAGS=${C_FLAGS}                                 \
        -D CMAKE_CXX_FLAGS=${CXX_FLAGS}                             \
        -D CMAKE_INSTALL_PREFIX=${COMPILE_INSTALL_ROOTS_DIRECTORY}    \
        -D CMAKE_BUILD_TYPE=${BUILD_TYPE}                           \
        -D gtest_dir=${GTEST_DIRECTORY}                             \
        -D UNIT_TEST=${UNIT_TEST}                                   \
        -D COVERAGE=${COVERAGE}                                     \
        -G "Unix Makefiles" ${ROOTS_DIRECTORY} | tee cmake_config.out

  ${MAKE_CMD}
  cd ${CURRENT_PATH}

  if [ ! -d "${COMPILE_INSTALL_ROOTS_DIRECTORY}" ]; then
    echo "ERROR:"
    echo "${COMPILE_INSTALL_ROOTS_DIRECTORY} does not exist."
    COMPILE_FAIL=1
  fi
  ...
  if [ ${COMPILE_FAIL} == 0 ]; then
    echo " "
    echo " ============================================================ "
    echo -e " ${gC} Roots build successful! ${eC}"
    echo    "   Compiling Options:"
    echo    "          Build Type: ${BUILD_TYPE}"
    echo    "      Build Location: ${COMPILE_BUILD_ROOTS_DIRECTORY}"
    ...
    echo " "
    echo         "================================"
    echo -e "${rC} Roots build FAILED! ${eC}"
    echo         "================================"
    echo " "
    exit 1
  fi
